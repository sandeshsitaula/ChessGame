...
class HomeWebSocketView(AsyncWebsocketConsumer):
        ...
        try:
            ...
            if data.get("type") == "send_challenge":
                ...
                if not challenge_response["status"]:
                    await self.send(
                        json.dumps(
                            {"type": "error", "message": challenge_response["message"]}
                        )
                    )
                    ...
                    json.dumps(
                        {
                            ...
                            "response": {"challenges": pending_challenges},
                        }
                    )
                )
            ...
            if data.get("type") == "accept_invite":
                ...
                await self.send(
                    json.dumps(
                        {"type": "accept_invite", "match_id": data.get("match_id")}
                    )
                )
                await self.channel_layer.group_send(
    ...
    async def accept_invite(self, event):
        await self.send(
            json.dumps({"type": "invite_accepted", "match_id": event["match_id"]})
        )
    ...
    def handle_accept_invite(self, data):
        ...
        current_match = Match.objects.filter(id=match_id, status="INVITED").first()

        current_match.status = "PENDING"
        current_match.save()

        # Create note for accepting player
        Note.objects.get_or_create(player=current_user, match=current_match)
...
class ChessWebSocketView(AsyncWebsocketConsumer):
        ...
        try:
            ...
            if incoming_data.get("type") == "chess_move":
                ...
                if not move_response["status"]:
                    await self.send(
                        text_data=json.dumps(
                            {"type": "error", "message": move_response["message"]}
                        )
                    )
                    return
                await self.channel_layer.group_send(
                ...
                if not response["status"]:
                    await self.send(
                        json.dumps({"type": "error", "message": response["message"]})
                    )
                print("response", response)
                await self.channel_layer.group_send(
                    self.group_name,
                    {"type": "forfeit_match", "forfeit_username": response["username"]},
                )
    ...
    async def forfeit_match(self, event):
        await self.send(
            text_data=json.dumps(
                {"type": "forfeit_match", "username": event["forfeit_username"]}
            )
        )
    ...
    def handle_forfeit_match(self):
        ...
        if not current_match.is_active:
            return {"status": False, "message": "Game is already over"}
    ...
    def handle_chess_move(self, data):
        ...
        return {"status": False, "message": "Illegal move"}
        """
